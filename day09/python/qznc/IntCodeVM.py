...
class IntCodeVM:
    def __init__(self, prog, inputs=None, outputs=None):
        self.prog = prog[:] # copy to avoid side effects
        self.prog.extend([0] * 1000) # enough?
    ...
    def _exec(self):
        pos = self.ip
        op, self.modes = op_parse(self.prog[pos])
        ...
        if op == 1: # add
            ...
            self._write(0, self.inputs.pop(0))
            return pos+2
        elif op == 4: # output
            val = self._read(0)
