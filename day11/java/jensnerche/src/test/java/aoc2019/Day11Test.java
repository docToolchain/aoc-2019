...
class Day11Test {
  ...
  @DisplayName("How many panels does it paint at least once? What registration identifier does it paint?")
  @ParameterizedTest
  @CsvSource({"part1, 2238","part2, 249"})
  void day11(String mode, int expectedPaintedPanels) throws InterruptedException {
    var hull = new int[10000][10000];
    var touched = new boolean[10000][10000];
    var rx = 5000;
    var ry = 5000;
    ...
    new Thread(() -> {
      execute(inputQueue, outputQueue, input);
      latch.countDown();
    }).start();

    if("part2".equals(mode)) hull[rx][ry] = 1;
    while(latch.getCount() > 0) {
      inputQueue.add((long) hull[rx][ry]);

      var color = outputQueue.poll(2, TimeUnit.SECONDS);
      if(color == null) break;
      var turn = outputQueue.take();

      hull[rx][ry] = Math.toIntExact(color);
      touched[rx][ry] = true;
    ...
    final int around = 80;
    for(var row = ry - around; row < ry + around; row++) {
      for(var col = rx - around; col < rx + around; col++) {
        System.out.print(hull[col][row] == 0 ? " " : "#");
      }
      System.out.println();
    }
    ...
    for(var row = ry - around; row < ry + around; row++) {
      for(var col = rx - around; col < rx + around; col++) {
        if(touched[col][row]) paintedPanels++;
      }
    }

    assertEquals(expectedPaintedPanels, paintedPanels);
  }
}
