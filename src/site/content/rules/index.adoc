...
You will notice that there is a lot to lear from other people's solutions!

Don't be shy and follow our inventation to our https://join.slack.com/t/aoc-2019/shared_invite/enQtNDg2NTI4NzY0Mjg5LTMzMDI1NzIyM2JiMzRhNGJhZTIwMWE4Y2Q3NmZmZjRlNWFhZDAwOWFkZDc0M2QxYTYzOGFmN2ZlZjIyYjNlZTU[AOC-2019 Slack channel].
It is the place to hamg around, talk about different solutions and language and get help if you are stuck.
...
As you can see, there is one folder per day (create one if you are the first for the day) and underneath this is onve folder per language used.
Then there is one fodler with your github handle which then contains your solution.

The file structure is quie important, because the build needs it to pick up the day, language and name in order to generate the webpage you currently read.
...
Have fun in solving some puzzles, but please remember it is not a competiotn and you don't have to solve all problems.
